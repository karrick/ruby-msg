  ...
  class PropertySet
    ...
    def body
      ...
      if !@body or @body.strip.empty?
        case message_class
        when 'IPM.Note.SMIME.MultipartSigned'
          Log.warn 'creating text body using new method for SMIME'
          # @body = msg.root.flatten.select {|x| x.file? }.map {|x| x.read}.grep(/^Content-Type/).first
          # data = msg_s.attachments.first.data.read
          @body = 'BOGUS'
        else
